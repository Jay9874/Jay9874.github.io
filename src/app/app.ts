...
import { createSpaceBackground, SpaceBackgroundHandle } from '../Utils/space';
import { createNebulaBackground, NebulaBackgroundHandle } from '../Utils/nebula';
...
export class App implements AfterViewInit, OnDestroy {
  ...
  // Capture the native element securely
  @ViewChild('box', { static: false }) boxElement!: ElementRef<HTMLDivElement>;

  // Get a reference to the starting path element
  @ViewChild('shadow', { static: true }) shadow!: ElementRef<SVGPathElement>;

  private tween?: gsap.core.Tween;
  private isMorphed = false;
  ...
  private spaceBackground!: NebulaBackgroundHandle;
  ...
  ngAfterViewInit(): void {
    ...
    // Run animation once the DOM element exists
    this.tween = gsap.to(this.boxElement.nativeElement, {
      x: 300,
      duration: 1.5,
      rotation: 360,
      ease: 'power2.out',
  ...
  ngOnDestroy(): void {
    ...
    // Kill the animation when the component is destroyed to prevent memory leaks
    if (this.tween) {
      this.tween.kill();
    }
  }
  ...
  private initThree(): void {
    ...
    this.spaceBackground = createNebulaBackground(this.scene);
  }

  morphShape() {
    const targetSelector = this.isMorphed ? '#start' : '#end';
    const targetColor = this.isMorphed ? '#FF5733' : '#33RMFF';

    gsap.to(this.shadow.nativeElement, {
      duration: 1,
      morphSVG: targetSelector, // Pass the selector of the target path
      fill: targetColor, // Optional: Animate color at the same time
      ease: 'power2.inOut',
    });
